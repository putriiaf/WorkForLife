...
<section class="mx-10">
    <div class="flex flex-row font-montserrat">
        ...
        <a href="/loker/{{ $loker->id }}/edit" class="bg-dongker px-8 py-2 mt-8 text-white block rounded-xl font-semibold uppercase tracking-wide hover:bg-dongker/70 hover:text-white/50">Edit</a>
        <form action="/loker/{{ $loker->id }}" method="post">
            @method('delete')
            @csrf
            <button class="bg-dongker px-8 py-2 mt-8 text-white block rounded-xl font-semibold uppercase tracking-wide hover:bg-dongker/70 hover:text-white/50" onclick="return confirm('Apakah Anda yakin ingin menghapus lowongan kerja ini?')">Hapus</button>
        </form>
        @endif
        @endif
    </div>
