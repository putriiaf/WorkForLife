...
@section('content')
    ...
    <div class="font-montserrat text-black">
        <h1 class="text-3xl text-center font-bold mb-5">Form Tambah Postingan</h1> <br> 
        <div class="container text-center bg-[#F6F6F6] mx-auto p-4 shadow-lg rounded">  
        <form method="POST" action="/uploadpost" class="" enctype="multipart/form-data">
            @csrf
            <div class="p-6 font-bold">
                <label class="text-blueGray-600 text-sm font-bold" for="judul">Judul</label>
                <input type="text" class="bg-white border border-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64" name="judul" value="{{ old('judul') }}">
            </div>
            <div class="font-bold">
                <label for="deskripsi" class="text-blueGray-600 text-sm font-bold">Deskripsi</label>
                <textarea id="deskripsi" class="bg-white border border-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 h-48"  type="text" name="deskripsi" value="{{ old('deskripsi') }}"></textarea>
            </div>
            @auth
